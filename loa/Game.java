...
class Game {
    ...
    /** Name of help html.*/
    static final String HELP_GUI = "loa/Help.html";
    ...
    /** A switch function switch machine player and human. */
    void switchSide() {
        Piece side = _board.turn();
        if (side == WP) {
            Piece opp = BP;
            manualCommand(opp.toString());
        } else {
            Piece opp = WP;
            manualCommand(opp.toString());
        }
        autoCommand(side.toString());
